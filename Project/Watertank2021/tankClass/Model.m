...
classdef Model < handle
    ...
    methods
        ...
        function data=ReadData(obj,controller)
            ...
            obj.History(end,1:4)=obj.state(1:4);
            ...
            obj.History(end,9:10) = obj.state(5:6);
        ...
        function WriteData(obj,data)
            ...
            if min(obj.state)< 0 || max(obj.state) > 110 % Hard saturation
                ...
                obj.state(obj.state(1:4) < 0) = 0;
                obj.state(obj.state(1:4) > 120) = 120;
            ...
            if isnan(obj.History(end-delay,1))
                ...
                % --- INTERFACE STATE --- %
                interface_state(1:4) = (25/100)*obj.state(1:4);% % -> cm
                interface_state(5:6) = (15/100)*obj.state(5:6); 
                % --- MODEL --- %
                ...
                delta1=obj.model.delta1;
                delta2=obj.model.delta2;
...
%%%% Need to update to 2 control signals
                interface_state(1,1) = interface_state(1,1) - obj.Ts*(alpha1*sqrt(2*g*interface_state(1,1)) - alpha13*sqrt(2*g*interface_state(1,3)) - beta1*obj.History_con(1,1)*(1+noise_level*randn)-beta1*interface_state(1,5)*1);
                interface_state(1,2) = interface_state(1,2) - obj.Ts*(alpha2*sqrt(2*g*interface_state(1,2)) - alpha24*sqrt(2*g*interface_state(1,4)) - beta2*obj.History_con(1,2)*(1+noise_level*randn)-beta2*interface_state(1,6)*1); %control u2
                interface_state(1,3) = interface_state(1,3) - obj.Ts*(alpha3*sqrt(2*g*interface_state(1,3)) - beta32*obj.History_con(1,2)*(1+noise_level*randn)-beta32*interface_state(1,6)*1); %control u2
                interface_state(1,4) = interface_state(1,4) - obj.Ts*(alpha4*sqrt(2*g*interface_state(1,4)) - beta41*obj.History_con(1,1)*(1+noise_level*randn)-beta41*interface_state(1,5)*1);
                interface_state(1,5) = interface_state(1,5) + obj.Ts*(-1/delta1*interface_state(1,5)+obj.History_con(1,1));
                interface_state(1,6) = interface_state(1,6) + obj.Ts*(-1/delta2*interface_state(1,6)+obj.History_con(1,2));
                % --- INTERFACE STATE --- %
                interface_state(1,1) = max([0 interface_state(1,1)]);
                interface_state(1,2) = max([0 interface_state(1,2)]);
                interface_state(1,3) = max([0 interface_state(1,3)]);
                interface_state(1,4) = max([0 interface_state(1,4)]);
                %interface_state(1,5) = 0;
                %interface_state(1,6) = 0;
                obj.state(1:4) = real((100/25)*interface_state(1,1:4)); % cm -> %
                obj.state(5:6) = real((100/15)*interface_state(1,5:6));
